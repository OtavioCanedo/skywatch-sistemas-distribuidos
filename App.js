...
export default function App() {
  ...
  async function fetchWeather() {
    ...
    setLoading(true);
    const options = {
      url: `https://meteostat.p.rapidapi.com/point/daily?lat=${city.lat}&lon=${city.lon}&start=2023-10-29&end=2023-10-30`,
      headers: {
  ...
  function buildCityOptions(data) {
    ...
    const options = data.map((city) => ({
      label: city.name,
      value: { name: city.city, lat: city.latitude, lon: city.longitude },
    }));
    ...
    <View style={styles.container}>
      ...
      <Card style={styles.card}>
        ...
        <Button
          ...
          style={styles.button(openDropDown)}
          loading={loading}
        >
          Ver Previsão
        </Button>
        ...
        {weather && (
          <View style={styles.weatherContainer}>
            ...
            <Text style={styles.weatherText}>
              Mínima: {weather.minTemperature} °C
            </Text>
            <Text style={styles.weatherText}>
              Máxima: {weather.maxTemperature} °C
            </Text>
            <Text style={styles.weatherText}>
              Precipitação: {weather.precipitation} mm
            </Text>
            <Text style={styles.weatherText}>
